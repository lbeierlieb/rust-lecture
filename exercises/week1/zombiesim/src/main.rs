...
impl World {
    ...
    fn new(rng: &mut Rng, population_fraction: f64) -> Self {
        let mut grid = [GridCell::Uninhabited; WORLD_SIZE];

        let mut index = 0;

        while index < WORLD_SIZE {
            if rng.next_f64() <= population_fraction {
                grid[index] = GridCell::Uninfected;
            }
            index += 1;
        }
    ...
    fn infect(&mut self, index: usize) -> bool {
        match self.grid[index] {
            GridCell::Uninfected => {
                self.grid[index] = GridCell::Infected;
                true
            }
            _ => false,
        }
    ...
    fn infect_random(&mut self, rng: &mut Rng) {
        loop {
            let index = rng.next_range(0, WORLD_SIZE as u64) as usize;

            if self.infect(index) {
                break;
            }
        }
    }
    ...
    fn infect_neighbors(&mut self, index: usize) -> u64 {
        ...
        if let Some(neighbor_coord) = coord.offset(0, -1) {
            if self.infect(neighbor_coord.to_index()) {
                infection_counter += 1;
            }
        }
        if let Some(neighbor_coord) = coord.offset(1, 0) {
            if self.infect(neighbor_coord.to_index()) {
                infection_counter += 1;
            }
        }
    ...
    fn simulate_infection(&mut self) {
        loop {
            ...
            while index < WORLD_SIZE {
                if let GridCell::Infected = self.grid[index] {
                    infection_counter += self.infect_neighbors(index);
                }
...
fn main() {
    ...
    while population <= population_max {
        ...
        while rep_counter < repetitions {
            let (uninfected, infected) = World::simulation_run(&mut rng, population);
